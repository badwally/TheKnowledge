...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_evaluate.add_argument("--max-chars", type=int, default=None, metavar="N",
                            help="Override the wiki context budget (default 500000). "
                                 "Use for large domains that exceed the default limit.")
    ...
    # contradiction-sweep (AGT-4)
    p_csweep = subparsers.add_parser("contradiction-sweep", help=SUBCOMMANDS["contradiction-sweep"])
...
def _run_evaluate(ns: argparse.Namespace) -> int:
    ...
    return _emit_result(
        evaluate_op(
            domain=ns.domain,
            limit=ns.limit,
            max_chars=getattr(ns, "max_chars", None),
...
def _run_contradiction_sweep_cmd(ns: argparse.Namespace) -> int:
...
def _run_fix_wikilinks_cmd(ns: argparse.Namespace) -> int:
