...
entity_kind: drug | person | paper | organization | dataset | product | software | statute | regulation | standard | place | event | artifact | other
...
Required sections: Summary, Key facts (with citations), Sources, Related. `last_verified_at` is required for `statute`, `regulation`, and `standard` kinds (lint ERROR if absent, WARNING if >365 days old — ONT-13).
