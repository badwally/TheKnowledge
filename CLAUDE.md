...
- **Run long multi-stream `/loop` work as fresh-session iterations — `/clear` (or a new session) between streams, not one continuous window.** Self-paced wakeups re-entering the *same* session accumulate context and pay a cold-cache reload on every post-5-minute fire (the cost compounds across iterations). The loop is fully resumable from `docs/session-state.md` (records the per-stream recipe + next atomic step), so clearing loses nothing; resume via the `contp` continuation prompt or by re-firing the loop clean. Single-window loops are fine only for short runs (≤2–3 iterations). For ingesting caller-vetted (e.g. deep-research-verified) sources, use `wiki ingest --force-include` to skip the strict filter's two-pass.
