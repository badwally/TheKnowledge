...
| M14 | _this commit_ | 222 | Real legacy migration phase 3 (edge-ai-agentic): 150 sources written + 213 pre-existing skipped (legacy edge_ai vault was a strict superset of phases 1+2 sources). Skip-if-canonical-exists guard added to source migration; cross-domain sources retain their original domain marker. 15/15 spot-check audit pass on phase-3-unique sources. 75 concepts + 8 MOCs + 4 synthesis added (concept/MOC/synthesis layer is fully disjoint from phases 1+2). Zero concept orphans across the 150 concepts now in canonical KB |
