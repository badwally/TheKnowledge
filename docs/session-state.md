...
- **C2 WAITING ON USER (next atomic step).** User labels TOMORROW: pick **10 best-fit video_ids overall** (across subtopics) + per-subtopic "expected but missing" flags. No formal rubric — independent human judgment is the ground truth by design (do NOT show the user policy.yaml criteria — keeps labels independent of what we test). Compact labeling sheet = `pool-scored.json` titles by subtopic (regenerate: group by subtopic, show `[video_id] title — channel`, hide score/tier).

### Next atomic step
**C2 → write `docs/research/youtube-filter-sl/labels-train.yaml`** from the user's 10 picks: `{best_fit: [<full youtube watch URLs>], missing: {subtopic: [str]}}`. Join key is `url` (convert the user's `[video_id]` tags → `https://www.youtube.com/watch?v=<id>`). Then run `PYTHONPATH=src /Users/andrewgrant/code/knowledge/.venv/bin/python -m gateway.cli filter-eval score semantic-models --scored docs/research/youtube-filter-sl/train-pool/pool-scored.json --labels docs/research/youtube-filter-sl/labels-train.yaml` → record precision@10 + buckets. THEN C3 derive improvements (filter-correct needs an INGESTED source_id — pool candidates aren't ingested; resolve per plan C3: ingest the labeled best-fit/false-positives first OR pin examples directly), C4 held-out validate on queries-validate.yaml (~50 pool), C5 write-up `results.md`. Before any worktree commit verify `git branch --show-current` == `exp/youtube-filter-supervised`.
