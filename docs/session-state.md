...
Last updated: 2026-06-21 (YouTube-filter supervised-improvement exercise — Phases A+B SHIPPED; C1 pool + C2 labels+score DONE on branch `exp/youtube-filter-supervised`; precision@10 (train, before) = 0.10; next = C3 improvements)
...
### Next atomic step
**C3 — derive + apply improvements (DESIGN DECISION — consult user on lever scope before editing `policy.yaml`).** Three levers per plan §C3: (1) FP → negative examples + dampen reasoning-theory/keynote-prestige bias; (2) FN → positive examples for applied repr-learning/semantic-layer/applied-construction + strengthen inclusion signals; (3) coverage gaps → query-register revision for the 3 subtopics. KNOWN FRICTION: `wiki filter-correct` needs an INGESTED source_id; pool candidates are NOT ingested — resolve per plan C3 (ingest labeled best-fit/FP first via `wiki ingest --force-include`, OR pin examples directly via example-bank path). Then C4 held-out validate on queries-validate.yaml (~50 pool), C5 write-up `results.md` (qualitative — 10-label gold set). Before any worktree commit verify `git branch --show-current` == `exp/youtube-filter-supervised`.
