...
Last updated: 2026-06-21 (YouTube-filter supervised-improvement exercise — Phases A+B SHIPPED; C1+C2+C3 DONE on branch `exp/youtube-filter-supervised`; precision@10 train 0.10 → 0.40 (all 10 picks accept-tier); next = C4 held-out validate)
...
### Phase C progress (operational, user-gated)
- **PR #34 OPEN** (https://github.com/badwally/TheKnowledge/pull/34) — Phases A+B (filter-eval tool + prompts). Code auto-merges clean vs main; only session-state.md conflicts.
- **C1 DONE — train pool generated + committed.** Ran `filter-eval pool semantic-models --queries queries-train.yaml --max-results 15`. **99 candidates / 8 subtopics / 53 accept-tier.** Live run validated end-to-end (probe: IBM "What is a KG?" → 0.0 reject, KGC keynotes → accept). Exact pool preserved at `docs/research/youtube-filter-sl/train-pool/{pool-blind.md,pool-scored.json}` (the /tmp copy is gone — use the committed one; spec §12 shared-pool protocol). Filter top-10 skews to Ian Horrocks/reasoning (7 of 10) — the disagreement the exercise measures.
- **C2 DONE — labels filed + filter scored.** User labeled via a Google Sheet (CSV→Sheet, clickable HYPERLINK watch col + scoring cols; IDs `1ChIBOCr-Qkqnb74qajUZZkQ-rVEWnSJ1IqSMGAsvzCw`). Only **X** marks counted as best_fit (user explicitly dropped two numeric cells `ww99npDh4cg`/`G-LVg8fWXtw`). Gold set committed at `docs/research/youtube-filter-sl/labels-train.yaml` (10 best_fit + 5 canonical-missing across 3 subtopics). **precision@10 (train, before) = 0.10** — only `83lI3u7KX0g` (OBO Foundry) overlaps. Disagreement is structural: filter top-10 collapses onto Ian Horrocks theory/keynote prestige (5 Horrocks + 2 keynotes); user rewards applied repr-learning / semantic-layer / applied construction breadth (picks ranked 18–89, the agentic-reasoning pick `DwFD` at rank 60). Buckets: 9 FP (theory/keynote over-weight), 9 FN (applied under-valued), coverage gaps kg-construction 2 / kg-embeddings 2 / ontology 1.

- **C3 DONE — improvements applied + train re-measured. precision@10 (train) 0.10 → 0.40; all 10 picks accept-tier (7→10).** Scope = examples + surgical policy edits (user-approved). Resolution path = direct example-bank pinning (NOT filter-correct — pool candidates un-ingested). Applied: (a) `policy.yaml` 6 surgical edits — new repr-learning/embeddings+KG-LLM inclusion criterion, semantic-layer & agentic-layer exclusion carve-outs, applied-depth content_depth positive, NeurIPS/KDD/ICLR+semantic-layer-summit authority; (b) 14 examples pinned (`docs/research/youtube-filter-sl/pin_examples.py`) — 10 picks as include/user-correction w/ de-anchored varied scores 0.80–0.95, 2 off-topic exclude/high-confidence, 2 over-valued-theory exclude/edge-case. Re-score harness `docs/research/youtube-filter-sl/rescore_train.py` (re-scores saved pool in place, no re-search). Result `train-pool/pool-scored-after.json`. **FINDING: examples did all the lift; policy edits redundant for train top-10 (kept for C4 generalization). FINDING: Haiku coarse scoring → score compression at top; de-anchored example scores helped spread (38-way tie at 0.85 → 13-way at 0.88). Ceiling: Horrocks/Labra-Gayo theory cluster holds 6 of top-10 at 0.88–0.93; pushing further overfits 10 labels + erodes KR recall.**
- **CRITICAL ENV GOTCHA:** `gateway.paths.knowledge_root()` defaults to `~/code/knowledge` (MAIN repo), NOT cwd/worktree. ALL filter ops (pin, load_policy, load_all) need `KNOWLEDGE_ROOT=/Users/andrewgrant/code/knowledge-wt-youtube-filter` exported or they read/write the main repo. First C3 run leaked 14 example files into main repo's tree (untracked) + used main's unedited policy — corrected by re-pinning under KNOWLEDGE_ROOT=worktree. **Main-repo leak cleanup is PENDING USER (blocked by auto-mode classifier): `rm -rf /Users/andrewgrant/code/knowledge/.knowledge/policies/semantic-models/examples/`.**

### Next atomic step
**C4 — held-out validation (real overfitting guard).** Run `KNOWLEDGE_ROOT=/Users/andrewgrant/code/knowledge-wt-youtube-filter PYTHONPATH=src .venv/bin/wiki filter-eval pool semantic-models --queries docs/research/youtube-filter-sl/queries-validate.yaml --max-results 15` (LIVE YouTube search — adapter idle precondition; key set, youtube adapter enabled). Uses improved policy+examples. ~50-candidate validate pool over 4 held-out subtopics (ontology-design-patterns, alignment-matching, upper-foundational-ontologies, obda-virtual-kg). Build a validate labeling Google Sheet for user (same format as train), user does lighter labeling → `labels-validate.yaml`, then `filter-eval score` → precision@10 (validate). THEN C5 results.md (qualitative; 10-label gold set). Before any worktree commit verify `git branch --show-current` == `exp/youtube-filter-supervised`.
