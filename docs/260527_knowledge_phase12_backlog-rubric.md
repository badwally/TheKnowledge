...
| `wiki nlm-register <domain> <notebook_id>` CLI | Code quality | **Active** — second occurrence in one day (commit 1a5d99b registered glp1-reward-modulation by direct YAML edit; orita-cmo will need the same). No CLI surface; only auto-create path exists via `_resolve_or_create_notebook` (`src/gateway/ops/nlm.py:63`), which spawns a NEW notebook instead of binding to an existing one | Dim 1: low daily-ops impact but recurrence is the trigger; S-effort thin wrapper over `nlm_registry.register()`; closes a documented hard-rule edge case (gateway ops vs direct edit) |
| `NoteConverter` for local `.md` and `.txt` files | Code quality | **Active** — encountered 2026-05-28 ingesting Orita seed corpus: 4 local `.md` docs required pandoc → `.docx` workaround; `.txt` transcript required PDF conversion before ingest. `note` is in `paths.SOURCE_TYPES` but no converter is registered (only pollers `apple_notes` / `notion` / `repo_metadata` write to `raw/note/`). Pattern recurs for any project ingesting local-markdown documentation | Dim 1: low daily-ops impact on wiki itself but recurring friction for any `~/code/*` cross-project ingest; S-effort wrapper (treat `.md` as canonical markdown with light frontmatter wrap, `.txt` as plain note body); closes documented gap between `note` source-type registration and converter registry |
