...
- **Bootstrap generates channel-authority signals automatically.** `wiki bootstrap-domain`'s policy-gen prompt should emit `channel_authority`/`speaker_expertise` when a domain is likely to draw on video sources. Larger change to the bootstrap LLM prompt; defer.
  - **RESOLVED 2026-06-17 (branch `feat/bootstrap-channel-authority`).** Added a conditional instruction to `_PROMPT_TEMPLATE` in `ops/bootstrap_domain.py`: when the model judges the new domain likely to draw on video/talk sources (lectures, conference talks, YouTube, podcast transcripts), it must emit `channel_authority` + `speaker_expertise` quality_signals; otherwise omit them. LLM-judged rather than keyword-detected (the model has the full description). Guidance is domain-agnostic — describes the signal *shape* (recognized institutions/venues/speakers vs influencer/marketing channels) without copying AI-specific examples, avoiding surface-anchor leakage from the agentic-data-layer policy. Tested at the prompt-construction layer (`test_bootstrap_prompt_instructs_channel_authority_for_video_domains`); 12/12 bootstrap + 50/50 policy tests pass.
- **Local-RAG transcript capture.** The wiki's `raw/` copy and `wiki retrieve` see only what the YouTube *converter* writes, not NLM's fetched transcript. Verify the converter captures the full transcript so local retrieval quality matches the NLM corpus. Independent of this fix.
  - **RESOLVED 2026-06-17 — verified no-op.** `converters/youtube.py` `convert()` already fetches the transcript via `_fetch_transcript` (youtube-transcript-api) and writes the full text to the `raw/` body as `[<seconds>] text` lines (`youtube.py:115-118`); only metadata sits in frontmatter. Empirical check: a fresh fetch of a known-captioned video (`arj7oStGLkU`) produced a 2,592-word / 450-line body spanning the whole talk (open → closing "(Applause)"). So local `wiki retrieve`/`answer`/FTS index the same transcript NLM gets via `source_add_url` — parity holds. The thin ~1.5KB `raw/youtube/*` files are pre-converter **legacy imports** (`source_app: legacy-research-notebook`, `legacy_recovery: summary-only`), not a converter regression. No code change.
