...
- **When a plan step names the execution/drain mechanism for a concurrency or contention test, verify that mechanism actually produces the contention the test targets** — a step that *looks* concurrent can serialize the very race it claims to drive. Test-harness-expansion P3: the brief's Step 1 said "fire concurrently via the real queue + `run_worker`", but `run_worker(once=True)` is a single-threaded sequential drainer — it claims→authors→commits each intent before touching the next, so two same-slug intents never contend for the commit lock and the CAS-contradiction path the test exists to exercise is unreachable (the outcome was a deterministic both-committed union every run). The plan was internally inconsistent: its own Global Constraint ("drive the REAL producer... the whole point of P3") and the P3 reviewer bar ("genuinely contend under the real lock, not serialized") demanded what Step 1's mechanism couldn't deliver. The fix mirrored the file's existing concurrent-`drain_once` soak harness. The reviewer caught it only by **instrumenting the actual outcome distribution** (5/5 union with the named drain; (committed, dead-lettered) under genuine concurrent drainers) — reading the test green proved nothing. Lesson: a contention test's drain mechanism is a *plan-time* claim to verify against the contention it must produce, and the higher-priority Global Constraint governs an inconsistent step.
...
- Test-Harness Expansion (6 tasks P1–P6, PR #38): produced the B2 bullet "verify a contention test's drain mechanism actually produces the contention it targets" (P3: `run_worker(once=True)` serialized the same-slug race the test existed to drive). Reinforced A1: every non-trivial task (P2/P3/P4) passed the implementer's own green tests but was BLOCKed by independent review for being inert — a lazily-xfailed deterministically-drivable lint check, a serialized "race", and a 5-value generator masquerading as a Hypothesis property. The meta-gate (P2) again surfaced real pre-existing production defects past green tests — two lint checks registered under a slug they don't emit (`citation-chains`→`dangling-synthesizes-ref`, `long-slugs`→`slug-too-long`), masked by empty-repo negative controls.
